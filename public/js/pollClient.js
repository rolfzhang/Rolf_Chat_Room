(function(exports){
    ...
    var globalEventHandler = {
        ...
        startLongPoll: function(){
            ...
            $.ajax({
                url: '/poll/stream?id='+(new Date).getTime(),
                success: function(data) {
                    if(data.result=='success'){
                        $.each(data.data, function(i,obj){
                            switch(obj.method){
                                case 'login':
                                    myEvents.trigger('addOnlinePeople',[obj.name]);
                                    break;
                                case 'logout':
                                    if(obj.name==nickname){
                                        myEvents.trigger('logout');
                                    }else{
                                        myEvents.trigger('removeOnlinePeople',[obj.name]);
                                    }
                                    break;
                                case 'receiveMessage':
                                    myEvents.trigger('showMsg',[obj.message]);
                                    break;
                            }
                        })
                    }else{
                        myEvents.trigger('errorHandle',['startLongPoll',data]);
                    }
                },
                complete:function(){
                    //递归，保持长连接
                    nickname && myEvents.trigger('startLongPoll');
                }
            })
        },
        //响应事件
        ...
        appStart: function(){
            ...
            //启动长连接
            myEvents.trigger('startLongPoll');
            console.log('start long poll');
